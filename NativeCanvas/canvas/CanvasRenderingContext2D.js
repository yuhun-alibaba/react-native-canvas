...
export default class CanvasRenderingContext2D extends CanvasRenderingContextStyle {
  ...
  draw() {
    if (this._canvas && this._canvas.update) {
      this._canvas.update(this._actions);
      this.resetActions();
    }
  }
  ...
  measureText(...args) {
    this.enqueue(this.createAction("measureText", args));
  }
  ...
  save(...args) {
    this.enqueue(this.createAction("save", args));
  }

  restore(...args) {
    this.enqueue(this.createAction("restore", args));
  }
}
